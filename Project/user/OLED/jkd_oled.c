...
#define CS_GPIO_Pin    GPIO_PIN_15  // PA15, 软件 CS
...
void OLED_W_D0(uint8_t BitValue)
{
	...
	if(BitValue)
		gpio_bit_set(GPIOx,SCL_GPIO_Pin);
	else
		gpio_bit_reset(GPIOx,SCL_GPIO_Pin);
...
void OLED_W_D1(uint8_t BitValue)
{
	...
	if(BitValue)
		gpio_bit_set(GPIOx,SDA_GPIO_Pin);
	else
		gpio_bit_reset(GPIOx,SDA_GPIO_Pin);
...
void OLED_W_RES(uint8_t BitValue)
{
	...
	if(BitValue)
		gpio_bit_set(GPIOx,RES_GPIO_Pin);
	else
		gpio_bit_reset(GPIOx,RES_GPIO_Pin);
}
...
void OLED_W_DC(uint8_t BitValue)
{
	...
	if(BitValue)
		gpio_bit_set(GPIOx,DC_GPIO_Pin);
	else
		gpio_bit_reset(GPIOx,DC_GPIO_Pin);
}
...
void OLED_W_CS(uint8_t BitValue)
{
	...
	if(BitValue)
		gpio_bit_set(GPIOA,CS_GPIO_Pin);
	else
...
void OLED_gpioInit(void)
{
    ...
    gpio_init(GPIOA, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, CS_GPIO_Pin);
    gpio_init(GPIOB, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, 
              RES_GPIO_Pin | DC_GPIO_Pin | SCL_GPIO_Pin | SDA_GPIO_Pin);
